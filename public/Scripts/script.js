...
let checkUsernameAvailability = async () => {
  let Username = document.getElementById("username").value;
  let reqBody = { username: Username };
  let response = await fetch("/username-avail", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
  ...
  if(data.length===0){
    document.getElementById("warning").classList.remove("hidden");
    document.getElementById("warning").innerHTML="UserName is not in database please enter valid username!"
    console.log(data);
  }
};
